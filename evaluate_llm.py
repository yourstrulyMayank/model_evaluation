...
#     # save_benchmark_result(model_name, all_results)
#     return all_results
...
#     overall_scores = [s for scores in task_type_results.values() for s in scores]
#     benchmark_mean = float(np.mean(overall_scores))
#     if overall_scores:
#         summary['overall'] = {
#             'mean': benchmark_mean,
...
#             allbenchhistory_data = []
#     allbenchresults = {
#         "BIG-Bench": benchmark_mean
#     }
#     # Create new entry
#     allbenchnew_entry = {
#         'model_path': model_path,
#         'benchmark': 'BIG-Bench',
#         'results': allbenchresults,
#         'timestamp': datetime.datetime.now().isoformat()
#     }
#     allbenchhistory_data.append(allbenchnew_entry)
...
#     """Save benchmark results to history.json"""
...
#     # FIXED: Save to allbenchmarkhistory.json with correct structure
#     allbenchhistory_file = "evaluation_results/allbenchmarkhistory.json"
...
#     # Create entry with correct structure that matches the HTML template
#     allbench_new_entry = {
#         'model_path': model_path,
#         'benchmark': 'BIG-Bench',        
#         'average': benchmark_percentage#,  # Added average field for template
#         # 'timestamp': datetime.datetime.now().isoformat()
#     }
...
# def save_benchmark_result(model_name, results, benchmark_name='BIG-Bench', timestamp=None):
#     """Save benchmark results to allbenchmarkhistory.json with correct structure"""
#     if timestamp is None:
#         timestamp = datetime.datetime.now().isoformat()
...
#     # Ensure results is a dict with benchmark scores
#     if isinstance(results, dict):
#         scores = results
#         # Calculate average if multiple benchmarks
#         benchmark_scores = [v for v in scores.values() if isinstance(v, (int, float))]
#         average = sum(benchmark_scores) / len(benchmark_scores) if benchmark_scores else 0
#     else:
#         # If results is a single score
#         scores = {benchmark_name: results}
#         average = results
...
#         'model_path': model_name,
#         'benchmark': benchmark_name,
#         'scores': scores,  # Changed from 'results' to 'scores'
#         'average': average,  # Added average field
#         'timestamp': timestamp
#     }
